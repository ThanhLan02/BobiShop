                                    ...
                                    @foreach ($users as $user)
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if ($user->image == null)
                                                        <img src="/storage/photos/1/users/No_Image_Available.jpg"
                                                            width="200px" height="200px" alt="user">
                                                    @else
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <div class="custom-control custom-switch">
                                                            <input type="hidden" name="_token"
                                                                value="{{ csrf_token() }}">
                                                            <input type="hidden" id="modelId{{ $user->id }}"
                                                                value="{{ $user->id }}">
                                                            <input type="checkbox" class="custom-control-input status"
                                                                id="customSwitch{{ $user->id }}" name="status"
                                                                value="{{ $user->status }}"
                                                                {{ $user->status == 'active' ? 'checked' : '' }}>
                                                            @if ($user->status == 'active')
                                                                <label class="custom-control-label"
                                                                    for="customSwitch{{ $user->id }}">Active</label>
                                                            @else
                                                                <label class="custom-control-label"
                                                                    for="customSwitch{{ $user->id }}">Inactive</label>
                                                            @endif
                                                        </div>
                                                    </div>
                                                </td>
                                                <td>
                                                    <a href="{{ route('admin.usersupdate', $user->id) }}"
                                                        class="btn btn-primary btn-sm float-left mr-1"
                                                        style="height:45px; width:50px;border-radius:40%"
                                                        data-toggle="tooltip" title="edit" data-placement="bottom"><i
                                                            class="fas fa-edit"></i></a>
                                                    <form method="POST"
                                                        action="{{ route('admin.usersdelete', $user->id) }}">
                                                        @csrf
                                                        @method('delete')
