                ...
                <div class="card-body">
                    <form method="post" action="{{ route('admin.productdoupdate', $product->id) }}">
                        ...
                        <div class="form-group">
                            ...
                            <input id="inputTitle" type="text" name="name" placeholder="{{ $product->name }}"
                        ...
                        <div class="form-group">
                            ...
                            <select name="category_id" id="category_id" class="form-control">
                                ...
                                @foreach ($categories as $key)
                                    <option value='{{ $key->id }}'
                                        {{ $product->category_id == $key->id ? 'selected' : '' }}>{{ $key->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('category_id')
                        ...
                        <div class="form-group">
                            ...
                            <select name="brand_id" id="brand_id" class="form-control">
                                ...
                                @foreach ($brands as $key)
                                    <option value='{{ $key->id }}'
                                        {{ $product->brand_id == $key->id ? 'selected' : '' }}>{{ $key->name }}
                                    </option>
                                @endforeach
                            </select>
                            @error('brand_id')
                        ...
                        <div class="form-group">
                            ...
                            <textarea style="height: 500px;" class="form-control" placeholder="Write something" id="description" name="description">{{ $product->description }}</textarea>
                            @error('description')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input id="old_price" type="number" name="old_price" min="0"
                                placeholder="Enter price" value="{{ $product->old_price }}" class="form-control">
                        </div>
                        <input id="new_price" type="number" name="new_price" min="0" placeholder="Enter price"
                            value="0" class="form-control" style="display: none;">
                        <div class="form-group">
